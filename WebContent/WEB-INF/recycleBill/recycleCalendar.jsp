<%@ page import="java.util.*, com.model.RecycleBill"%>
	...
	<c:otherwise>
		<div class="calendarContainer">
			<c:forEach var="i" begin="0" end="11" varStatus="loop">
				...
				<c:if
					test="${recycleBillStatusMap ne null and recycleBillStatusMap[month] ne null}">

					<c:set var="status" value="${recycleBillStatusMap[month]}" />
				</c:if>
					...
					<div class="monthFunc ${cssClass}">
						<c:choose>
							<c:when
								...
								<a class="uploadBtn"
									href="${pageContext.request.contextPath}/bills/recycleBill/${selectedYear}/${monthArray[i]}">Upload
									Bill</a>
							</c:when>
							<c:when
								...
								<a class="uploadBtn"
									href="${pageContext.request.contextPath}/bills/editRecycleBill/${selectedYear}/${monthArray[i]}">Edit
									Bill</a>
							</c:when>
							<c:when
								...
								<a class="uploadBtn"
									href="${pageContext.request.contextPath}/bills/editRecycleBill/${selectedYear}/${monthArray[i]}">ReUpload
									Bill</a>
							</c:when>
							<c:when
								test="${(i + 1 lt currentMonth or selectedYear lt currentYear) and status eq 'approve'}">
								<a class="uploadBtn"
									href="${pageContext.request.contextPath}/bills/recycleMonthReport/${selectedYear}/${monthArray[i]}">View
									Report</a>
							</c:when>
